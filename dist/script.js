...
/* harmony import */ var _modules_imageBlocks__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./modules/imageBlocks */ "./src/js/modules/imageBlocks.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_imageBlocks__WEBPACK_IMPORTED_MODULE_8__["default"])('.sizes-block');
...
var accordion = function accordion(btnTriggers, contentBlocks) {
  var btns = document.querySelectorAll(btnTriggers),
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var burger = function burger(burgerTrigger, burgerContent) {
  var menuBtn = document.querySelector(burgerTrigger),
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var calc = function calc(picSize, picMaterial, picOptions, picDiscount, picTotal) {
  var size = document.querySelector(picSize),
...
var drop = function drop() {
  ...
  ;
  ['dragenter', 'dragover'].forEach(function (eventType) {
    inputs.forEach(function (input) {
  ...
  ['dragleave', 'drop'].forEach(function (eventType) {
    inputs.forEach(function (input) {
      input.addEventListener(eventType, function () {
...
var forms = function forms() {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      arr[0].length > 6 ? dots = '...' : dots = '.';
      item.previousElementSibling.textContent = "".concat(arr[0].substring(0, 6)).concat(dots).concat(arr[1]);
    });
  });
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var api;
      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.design : api = path.server;
...
var imageBlocks = function imageBlocks(selector) {
...
/* harmony default export */ __webpack_exports__["default"] = (imageBlocks);
...
var mask = function mask(selector) {
  ...
  var inputs = document.querySelectorAll(selector);
  inputs.forEach(function (input) {
    input.addEventListener('input', createMask); // input.addEventListener('keypress', createMask);
...
var modal = function modal() {
  ...
  function bindModals(triggerSelector, modalSelector, closeSelector) {
    var destroyElem = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
...
var scroll = function scroll(upSelector) {
  ...
  var links = document.querySelectorAll('[href^="#"]'),
      //получаем все ссылки на лендинг - каретка указывает, что название     начитается с #//
  speed = 0.1;
  links.forEach(function (link) {
    link.addEventListener('click', function (e) {
      ...
      hash = this.hash,
          ...
          //получаем блоки через название хеши и сразу через свойство топ этого метода (у него много свойств, касательно положения элемента) получаем расстояние от экрана (которое может быть и отрицательным, если мы уже проскроллили этот блок) до блока
      start = null; //просто что бы запустить, в функции переназначается

      requestAnimationFrame(step);

      function step(time) {
        //time приходит автоматически - время от загрузки страницы (почему приходит само - пока хз)
        if (start === null) {
          start = time;
        }
...
var showMoreStyles = function showMoreStyles(trigger, area) {
  var btn = document.querySelector(trigger),
      section = document.querySelector(area);
  btn.addEventListener('click', function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResources"])('http://localhost:3000/styles').then(function (res) {
      createCards(res);
      btn.remove();
    }).catch(function (error) {
      var errMessage = document.createElement('div');
      errMessage.style.textAlign = 'center';
      errMessage.textContent = "\u041F\u0440\u043E\u0438\u0437\u043E\u0448\u043B\u0430 \u043E\u0448\u0438\u0431\u043A\u0430 (".concat(error, ")");
...
var sliders = function sliders(sliderContentSelector, sliderDirection, prevButton, nextButton) {
  var slider = document.querySelectorAll(sliderContentSelector),
      direction = sliderDirection;
  var activeSliderIndex = 1;
  ...
  function activateAnimation() {
    if (direction === 'vertical') {
      animationPaused = setInterval(function () {
        activeSlider(1);
...
var tabs = function tabs() {
  ...
  function showTabsContent(selector) {
    ...
    if (selector) {
      wrapper.querySelectorAll(selector).forEach(function (item) {
